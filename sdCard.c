...
int MSD0_Init(void)
{
   ...
   */ 
    ...
    MSD0_card_enable();    // cs low
    usart_sendString(USART2, "start\r\n");
    
    /* Start send CMD0 till return 0x01 means in IDLE state */
    for(retry=0; retry<0xFFF; retry++)
    {
        r1 = MSD0_send_command(CMD0, 0, 0x95);      // try this comand for manytimes
        if(r1 == 0x01)
        {
            retry = 0;
            usart_sendString(USART2, "cmd0 ok\r\n");
            break;
        }
    }
    ...
    {
        #ifdef PRINT_INFO 
        usart_sendString(USART2, "Reset card into IDLE state failed!\r\n");
        #endif
        return 1;
    }
    ...
    if(r1 == 0x05)
    {
        ...
        for(retry=0; retry<0xFFF; retry++)
        {
            ...
            {
                #ifdef PRINT_INFO 
                usart_sendString(USART2, "Send CMD55 should return 0x01");
                #endif
                return r1;
            }
        ...
        if(retry == 0xFFF)
        {
            for(retry=0; retry<0xFFF; retry++)
            {
                r1 = MSD0_send_command(CMD1, 0, 0);		/* should be return 0x00   try for many times*/
                if(r1 == 0x00)
                {
                    retry = 0;
                    break;
            ...
            {
                #ifdef PRINT_INFO
                usart_sendString(USART2,"Send CMD1 should return 0x00, response=0x%02x\r\n");
                #endif
                return 2;
            }	
            ...
            #ifdef PRINT_INFO 
            usart_sendString(USART2,"Card Type                     : MMC\r\n");
            #endif 				
        }	
        ...
        {
            usart_sendString(USART2,"Card Type                     : SD V1\r\n");
        }
        #endif 
        ...
        {
            #ifdef PRINT_INFO 
            usart_sendString(USART2,"Send CMD59 should return 0x00, response=0x%02x\r\n");
            #endif
        ...
        {
            #ifdef PRINT_INFO
            usart_sendString(USART2,"Send CMD16 should return 0x00, response=0x%02x\r\n");
            #endif
            return r1;		/* response error, return r1 */
        }
    }	
   ...
   else if(r1 == 0x01)
   {
        ...
        buff[0] = spi1_sendByte(DUMMY_BYTE);				/* should be 0x00 */
        buff[1] = spi1_sendByte(DUMMY_BYTE);				/* should be 0x00 */
        buff[2] = spi1_sendByte(DUMMY_BYTE);				/* should be 0x01 */
        buff[3] = spi1_sendByte(DUMMY_BYTE);				/* should be 0xAA */
	...
	if(buff[2]==0x01 && buff[3]==0xAA)
	{
            for(retry=0; retry<0xFFF; retry++)
            {
                ...
                {
                    #ifdef PRINT_INFO 
                    usart_sendString(USART2," Send CMD55 should return 0x01, BUT DOT\r\n");
                    #endif
                    return r1;
                }				
                ...
                {
                        retry = 0;
                        usart_sendString(USART2,"CMD41 ok\r\n");
                        break;
                }
            }
            ...
            {
                #ifdef PRINT_INFO
                usart_sendString(USART2,"Send ACMD41 should return 0x00, response=0x%02x\r\n");
                #endif
                return 3;
            }
	    ...
	    {
                #ifdef PRINT_INFO
		usart_sendString(USART2,"Send CMD58 should return 0x00, response=0x%02x\r\n");
                #endif
                return r1;		/* response error, return r1 */
	    }

            usart_sendString(USART2,"CMD58 ok\r\n");
            
	    buff[0] = spi1_sendByte(DUMMY_BYTE);					
            buff[1] = spi1_sendByte(DUMMY_BYTE);					
            buff[2] = spi1_sendByte(DUMMY_BYTE);					
            buff[3] = spi1_sendByte(DUMMY_BYTE);					

            /* End of CMD58, chip disable and dummy byte */
            MSD0_card_disable();
            spi1_sendByte(DUMMY_BYTE);   // 清除收发寄存器的数据
	    ...
	    if(buff[0] & 0x40)
	    {
                ...
                #ifdef PRINT_INFO 
		   usart_sendString(USART2,"Card Type                     : SD V2HC\r\n");
                #endif 	
	    }
	    else
	    {
                ...
                #ifdef PRINT_INFO
                  usart_sendString(USART2,"Card Type                     : SD V2\r\n");
                #endif 	
	    }
        }	
    }
...
int MSD0_GetCardInfo(PMSD_CARDINFO SD0_CardInfo)
{
    ...
    SD0_CardInfo->Capacity = 7948206080;//SD0_CardInfo->CSD.DeviceSize
...
int MSD0_read_buffer(uint8_t *buff, uint16_t len, uint8_t release)
{
    ...
    {
       *(buff+retry) = spi1_sendByte(DUMMY_BYTE);
    }
...
int MSD0_WriteSingleBlock(uint32_t sector, uc8 *buffer)
{
  ...
  retry = 0;
  while(spi1_sendByte(DUMMY_BYTE) == 0x00)
  {	
	 /* Timeout return */
	 if(retry++ == 0x40000)
	 {
...
int MSD0_WriteMultiBlock(uint32_t sector, uc8 *buffer, uint32_t NbrOfSector)
{
  ...
  {	
	 ...
	 /* Wait all the data programm finished	*/
	 retry = 0;
	 while(spi1_sendByte(DUMMY_BYTE) != 0xFF)
	 {	
		/* Timeout return */
		if(retry++ == 0x40000)
		{
...
int MSD0_send_command(uint8_t cmd, uint32_t arg, uint8_t crc)
{
  ...
  spi1_sendByte(cmd | 0x40);
  spi1_sendByte(arg >> 24);
  spi1_sendByte(arg >> 16);
  spi1_sendByte(arg >> 8);
  spi1_sendByte(arg);
  spi1_sendByte(crc);
