...
{	 
  ...
  else
  {
    ...
    {
      usart_debugMessage("Hub is not supported.\n");
    }
    
    else if (pphost->device_prop.Itf_Desc[0].bInterfaceClass == USB_CDCC)
    {
      usart_debugMessage("Communications and CDC Control device is not supported.\n");
    }
    
    else if (pphost->device_prop.Itf_Desc[0].bInterfaceClass == USB_HID)
    {
      usart_debugMessage("HID device is not supported.\n");
    }
    
    else if (pphost->device_prop.Itf_Desc[0].bInterfaceClass == USB_PRINTER)
    {
      usart_debugMessage("Printer device is not supported.\n");
    }
    
    else if (pphost->device_prop.Itf_Desc[0].bInterfaceClass == USB_SMARTCARD)
    {
      usart_debugMessage("Smart Card device is not supported.\n");
    }
    
    
    else if (pphost->device_prop.Itf_Desc[0].bInterfaceClass == USB_VIDEO)
    {
      usart_debugMessage("Video device  is not supported.\n");
    }
    
    
    else if (pphost->device_prop.Itf_Desc[0].bInterfaceClass == USB_AVD)
    {
      usart_debugMessage("Audio/Video Devices is not supported.\n");
    }
    
    else
    {
      usart_debugMessage ("The attached device is not supported. \n");
    }
...
static USBH_Status USBH_MSC_Handle(USB_OTG_CORE_HANDLE *pdev ,   //tocheck
                                   void   *phost)
{
  USBH_HOST *pphost = phost;
